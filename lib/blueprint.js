...
const sequence            = require('promise-map-series');
...
*/
const Blueprint = module.exports = CoreObject.extend({
  isBlueprint: true,
  availableOptions: [],
  ...
  */
  _process(options, beforeHook, process, afterHook) {
    ...
    return this._locals(options).then(locals => {
      return RSVP.Promise.resolve()
        ...
        .then(process.bind(this, intoDir, locals)).then(value => {
          return RSVP.map(value, this._commit.bind(this))
  ...
  install(options) {
    ...
    return this._process(
      ...
      this.beforeInstall,
      this.processFiles,
      this.afterInstall).finally(() => logger.info('END: processing blueprint: `%s` in (%dms)', this.name, new Date() - start));
  },
  ...
  */
  processFiles(intoDir, templateVariables) {
    ...
    return RSVP.filter(fileInfos, isValidFile)
      .then(value => RSVP.map(value, prepareConfirm))
  ...
  */
  ...
  getJson(verbose) {
    ...
    printableProperties.forEachWithProperty(key => {
      let value = this[key];
  ...
  /**
    ...
    @public
  */
  lookupBlueprint(dasherizedName, options) {
    let projectPaths = this.project ? this.project.blueprintLookupPaths() : [];

    if (typeof options === 'object' &&
        object !== null &&
        Array.isArray(options.paths)) {
      projectPaths.push.apply(projectPaths, options.paths);
    }
...
Blueprint.lookup = function(name, options) {
  if (!(options && Array.isArray(options.paths))) {
    throw new TypeError('Blueprint.lookup(name, { paths: [/* required */]})');
  }

  let lookupPaths = options.paths;
  let lookupPath;
  ...
  if (fs.lstatSync(blueprintPath).isDirectory()) {
    ...
    if (existsSync(constructorPath)) {
      ...
      if (typeof blueprintModule === 'function') {
        ...
        Constructor = this.extend(blueprintModule);
      }
    }
...
function isFile(info) {
  return stat(info.inputPath).then(value => value.isFile());
}
...
function isIgnored(info) {
  return _.any(Blueprint.ignoredFiles, ignoredFile => {
    return minimatch(info.inputPath, ignoredFile, { matchBase: true });
  });
}
...
function inRepoAddonExists(name, root) {
  return existsSync(path.join(root, 'lib', name));
}
...
function finishProcessingForInstall(infos) {
  ...
  return sequence(infosNeedingConfirmation, result => result()).then(() => infos);
}
