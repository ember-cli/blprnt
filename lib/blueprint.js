...
var sequence            = require('promise-map-series');
...
var Blueprint = CoreObject.extend({
  ...
  _process: function(options, beforeHook, process, afterHook) {
    ...
    return this._locals(options).then(function (locals) {
      return RSVP.Promise.resolve()
        ...
        .then(process.bind(self, intoDir, locals)).then(function(value) {
          return RSVP.map(value, self._commit.bind(self))
  ...
  */
  processFiles: function(intoDir, templateVariables) {
    ...
    return RSVP.filter(fileInfos, isValidFile).then(function(value) {
      return RSVP.map(value, prepareConfirm);
  ...
  */
  ...
  /**
    ...
    @public
  */
  lookupBlueprint: function(dasherizedName, options) {
...
function isFile(info) {
  return stat(info.inputPath).then(function(value) { return value.isFile(); });
}
...
function finishProcessingForInstall(infos) {
  ...
  return sequence(infosNeedingConfirmation, function() { }).then(function() {
    return infos;
  });
}
