...
module.exports = function cleanRemove(fileInfo) {
  // see if file exists to avoid wasting time
  return stat(fileInfo.outputPath)
    .then(() => {
      return remove(fileInfo.outputPath).then(() => {
        var paths = walkUp(fileInfo.displayPath).map(thePath => {
          return path.join(fileInfo.outputBasePath, thePath);
        });

        return paths.reduce((chainedPromise, thePath) => {
          return chainedPromise.then(wasShortCircuited => {
